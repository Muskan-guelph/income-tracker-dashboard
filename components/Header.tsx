...
const Header: React.FC<HeaderProps> = ({ isDarkMode, toggleTheme, currentPage, onNavigate }) => {
  return (
    <header className="px-8 py-6 flex items-center justify-between w-full relative z-20">

      {/* Left Tabs */}
      <div className={`flex items-center space-x-1 backdrop-blur-md p-1.5 rounded-full border shadow-2xl transition-all duration-300 ${isDarkMode
          ? 'bg-[#1a1a24]/80 border-white/[0.08]'
          : 'bg-white/80 border-slate-200 shadow-slate-200/50'
        }`}>
        <TabButton
          active={currentPage === 'home' || currentPage === 'companies' || currentPage === 'company-details' || currentPage === 'transactions'}
          text="Dashboard"
        ...
        />
        <TabButton text="Settings" icon={<Settings size={16} />} isDarkMode={isDarkMode} onClick={() => { }} />
      </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <button
          onClick={toggleTheme}
          className={`w-10 h-10 rounded-full border flex items-center justify-center transition-all duration-300 shadow-md group ${isDarkMode
        ...
        {/* Add Income Button */}
        <button className={`flex items-center space-x-2 px-5 py-2.5 rounded-full border text-sm font-medium transition-all duration-300 group shadow-lg hover:shadow-xl active:scale-95 ${isDarkMode
        ...
        <div className="relative group cursor-pointer">
          <div className={`w-11 h-11 rounded-full overflow-hidden border-2 transition-all duration-300 p-0.5 shadow-lg ${isDarkMode
...
const TabButton: React.FC<{ text: string; icon: React.ReactNode; active?: boolean; isDarkMode: boolean; onClick: () => void }> = ({ text, icon, active, isDarkMode, onClick }) => {
  return (
    <button
      onClick={onClick}
      className={`flex items-center space-x-2 px-5 py-2.5 rounded-full text-xs font-medium transition-all duration-300 ${active
          ? isDarkMode
