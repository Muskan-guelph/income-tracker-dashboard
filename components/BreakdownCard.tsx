...
const BreakdownCard: React.FC<BreakdownCardProps> = ({ breakdown, isDarkMode }) => {
  ...
  return (
    <div className={`h-full p-8 rounded-[32px] backdrop-blur-xl border shadow-2xl flex flex-col relative overflow-hidden transition-all duration-500 ${
      isDarkMode 
      ...
      <div className="flex justify-between items-start mb-6 relative z-10">
        <div>
            ...
            <div className="flex items-center space-x-2 mt-2">
                ...
                {!isEmpty && (
                    <span className={`text-xs font-medium tracking-wide uppercase px-2 py-1 rounded-md ${
                        isDarkMode ? 'text-gray-500 bg-white/5' : 'text-slate-500 bg-slate-100'
                    }`}>Next Income: 17</span>
                )}
            </div>
        </div>
        <div className={`p-2 rounded-full border shadow-inner group cursor-pointer transition-colors ${
            isDarkMode 
      ...
      {isEmpty ? (
        ...
        <>
            ...
            <div className="flex flex-wrap gap-3 mb-2 relative z-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <Badge 
                    label="CPP" 
            ...
            <div className="flex-1 relative flex items-center justify-center min-h-[250px] animate-in zoom-in-95 duration-700">
                ...
                {/* Floating Labels (Taxes & Net) */}
                <div className="absolute top-[5%] right-0 flex flex-col items-end space-y-6 z-10 pointer-events-none">
                    <FloatingLabel 
                        label="Federal" 
            ...
            {/* Footer Info Cards */}
            <div className={`mt-auto border-t pt-3 animate-in fade-in slide-in-from-bottom-8 duration-700 ${isDarkMode ? 'border-white/[0.06]' : 'border-slate-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                    <LegendTextItem 
                        label="Federal" 
                        value={`$${breakdown.federalTax.toLocaleString(undefined, {minimumFractionDigits: 2})}`} 
                        percent={`${getPercent(breakdown.federalTax)}%`}
                        color="bg-[#8b5cf6]" 
                        isDarkMode={isDarkMode}
                    />
                    <LegendTextItem 
                        label="Provincial" 
                        value={`$${breakdown.provincialTax.toLocaleString(undefined, {minimumFractionDigits: 2})}`} 
                        percent={`${getPercent(breakdown.provincialTax)}%`}
                        color="bg-[#d946ef]" 
                        isDarkMode={isDarkMode}
                    />
                    <LegendTextItem 
                        label="CPP" 
                        value={`$${breakdown.cpp.toLocaleString(undefined, {minimumFractionDigits: 2})}`} 
                        percent={`${getPercent(breakdown.cpp)}%`}
                        color="bg-[#ec4899]" 
                        isDarkMode={isDarkMode}
                    />
                    <LegendTextItem 
                        label="EI" 
                        value={`$${breakdown.ei.toLocaleString(undefined, {minimumFractionDigits: 2})}`} 
                        percent={`${getPercent(breakdown.ei)}%`}
                        color="bg-[#be185d]" 
                        isDarkMode={isDarkMode}
                    />
                    <LegendTextItem 
                        label="Vacation Pay" 
                        value={`$${breakdown.vacationPay.toLocaleString(undefined, {minimumFractionDigits: 2})}`} 
                        percent={`${getPercent(breakdown.vacationPay)}%`}
                        color="bg-[#06b6d4]" 
                        isDarkMode={isDarkMode}
                    />
                </div>
...
const Badge: React.FC<{ label: string; percent: string; value: string; color: string; shadowColor: string; isDarkMode: boolean }> = ({ label, percent, value, color, shadowColor, isDarkMode }) => (
    <div className={`flex items-center space-x-2 px-3 py-1.5 rounded-full border shadow-lg backdrop-blur-md ${
        isDarkMode 
